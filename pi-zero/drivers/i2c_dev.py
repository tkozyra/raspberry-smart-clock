...
class I2CDevice:
    ...
    def set_custom_char(self, address, character):
        self.bus.write_byte(self.addr, 0b1100 | 0b01000000 | (address & 0x30))
        sleep(0.0001)
        self.bus.write_byte(self.addr, 0b1000 | 0b01000000 | (address & 0x30))
        sleep(0.0001)
        self.bus.write_byte(self.addr, 0b1100 | (address & 0x0F) << 4)
        sleep(0.0001)
        self.bus.write_byte(self.addr, 0b1000 | (address & 0x0F) << 4)
        sleep(0.0001)
        
        for i in range(0,len(character)):
            code = character[i]
            self.bus.write_byte(self.addr, 0b1101 | (code & 0xF0))
            sleep(0.0001)
            self.bus.write_byte(self.addr, 0b1001 | (code & 0xF0))
            sleep(0.0001)
            self.bus.write_byte(self.addr, 0b1101 | (code & 0x0F) << 4)
            sleep(0.0001)
            self.bus.write_byte(self.addr, 0b1001 | (code & 0x0F) << 4)
            sleep(0.0001)
...
class Lcd:
    ...
    def set_custom_char(self, address, character):
        self.lcd.set_custom_char(address, character)
